...
class SentimentTreeBank(object):
    ...
    def __init__(self, path="stanfordSentimentTreebank", split_ratios=(0.8,0.1,0.1), split_words=True):
        ...
        sentences = self._read_sentences() # returns a list of sentences
        self.sentences = self._build_dataset(sentences) # returns a list of Sentence objects (each one contains a tree)
        if self.split_words: # always true
...
if __name__ == '__main__':
    ...
    print(list(dataset.get_word_counts().items())[:10])
